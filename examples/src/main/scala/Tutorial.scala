...
@main def tutorial() =
  ...
  Sim.plotTs(
    ts2,
    "Gillespie simulation of LV model with non-default parameters"
  )
  ...
  val tsi =
    Sim.times(DenseVector(50, 100), 0.0, List(0.0, 2.0, 5.0, 10.0, 20.0), step)
  ...
  def lvparam(
      p: DenseVector[Double] = DenseVector(1.0, 0.005, 0.6)
  ): Spn[IntState] =
  ...
  val tslv1 =
    Sim.ts(DenseVector(50, 100), 0.0, 20.0, 0.05, Step.gillespie(mylv1))
  Sim.plotTs(tslv1, "Gillespie simulation of LV1")

  val mylv2 = lvparam(DenseVector(1.1, 0.01, 0.6))
  val tslv2 =
    Sim.ts(DenseVector(50, 100), 0.0, 20.0, 0.05, Step.gillespie(mylv2))
  ...
  def lv[S: State](
    ...
    UnmarkedSpn[S](
      ...
      (x, t) =>
        val xd = x.toDvd
        DenseVector(xd(0) * p(0), xd(0) * xd(1) * p(1), xd(1) * p(2))
    )

  val mylv3: Spn[IntState] = lvparam() // here due to compiler bug!
  val tslv3 =
    Sim.ts(DenseVector(50, 100), 0.0, 20.0, 0.05, Step.gillespie(mylv3))
  Sim.plotTs(tslv3, "Gillespie simulation of LV3")

  val lvDiscrete = lv[IntState]()
  val tsDiscrete =
    Sim.ts(DenseVector(50, 100), 0.0, 20.0, 0.05, Step.gillespie(lvDiscrete))
  Sim.plotTs(tsDiscrete, "Gillespie simulation of lvDiscrete")

  val lvDiscrete2 = lv[IntState](DenseVector(1.1, 0.01, 0.6))
  val tsDiscrete2 =
    Sim.ts(DenseVector(50, 100), 0.0, 20.0, 0.05, Step.gillespie(lvDiscrete2))
