...
object Mll {
  ...
  /**
    * A simple bootstrap particle filter with multinomial resampling. Note that it
    * supports time series of observations on irregular time grids. Note that this is
    * the function which does most of the work for `pfMll`.
    * 
    * @param x0 A sample from the prior on the state at time `t0`. The size of the sample
    * determines the number of particles used in the particle filter.
    * @param t0 The time corresponding to the prior sample
    * @param data The observed data in the form of a time series
    * @param dataLik The log-likelihood of the observation model
    * @param stepFun The transition kernel of the underlying process
    * 
  ...
  /**
    ...
    * 
    * @param simX0 A function which when provided with a parameter value will return a sample from the prior on the intial state of a Markov process model. The size of the sample will determine the number of particles used in the particle filter.
    * @param t0 The intial time corresponding to the prior sample
    * @param stepFun A function, which when provided with a parameter value, will return the transition kernel of a Markov process.
