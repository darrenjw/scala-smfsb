...
package object smfsb {
  ...
  /** The main time series class, for representing output from simulation
    * functions, and for observed time course data
    */
  type Ts[S] = List[(Time, S)]

  /** Type representing log-likelihoods - just an alias for `Double`. All
    * likelihoods in this library are on a log scale. There should be no raw
    * likelihoods passed into or out of any user-facing function.
    */
  type LogLik = Double
  ...
  /** Type class for vectors that can be rendered to a CSV string (and a Breeze
    * DenseVector[Double]), extended by `State`
    */
  trait CsvRow[T] {
  ...
  /** State type class, with implementations for Breeze `DenseVector` `Ints` and
    * `Doubles`
    */
  ...
  implicit val dvdState: State[DoubleState] = new State[DoubleState] {
    def toCsv(s: DoubleState): String =
      (s.toArray map (_.toString)).mkString(",")
  ...
  /** Comonadic pointed vector type (parallel implementation), used by the
  ...
  case class PVector[T](cur: Int, vec: ParVector[T]) {
    ...
    def coflatMap[S](f: PVector[T] => S): PVector[S] =
      PVector(
        cur,
        (0 until vec.length).toVector.par.map(i => f(PVector(i, vec)))
      )
  ...
  case object PMatrix {
    ...
    /** Constructor for `PMatrix` objects. Used for initialising 2d spatial
      * simulations.
      */
